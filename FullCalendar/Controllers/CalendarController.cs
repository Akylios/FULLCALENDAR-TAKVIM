...
namespace FullCalendar.Controllers
{
    public class CalendarController : Controller
    {
        ...
        public JsonResult GetCalendarEvents(string start, string end)
        {
            ...
            try
            {
                ...
                for (i = 0; i < n; i++)
                {
                    ...
                    item.allDay = string.IsNullOrEmpty(dr["AllDay"].ToString()) ? true : false;
        ...
        [HttpPost]
        public JsonResult KayitGoster(int id)
        {
            DBConnection connect = new DBConnection();
            CalendarEvent takvim = new CalendarEvent();
            try
            {
                connect.OpenConnection();
                List<SqlParameter> param = new List<SqlParameter>();
                ...
                {
                    SqlConnection connect2 = new SqlConnection(DBConnection.connectionString);
                    string sql = "Select * from tCalendar Where IDCalendar=" + id;
                    connect2.Open();
                    // param.Add(new SqlParameter("@IDCalendar", id));
                    SqlCommand sc = new SqlCommand(sql, connect2);
                    SqlDataReader dr = sc.ExecuteReader();
                    if (dr.Read())
                    {
                        takvim.id = id;
                        takvim.title = dr["Title"].ToString();
                        takvim.start = dr["StartDate"].ToString();
                        takvim.end = dr["EndDate"].ToString();
                        takvim.color = dr["Color"].ToString();
                ...
                return Json(takvim, JsonRequestBehavior.AllowGet);
            }
            finally
            {
                connect.CloseConnection();
